...
export function createInitialBoard(): ChineseChessPiece[] {
  ...
  // Tướng
  pieces.push(createPiece("black-general", "general", "black", 0, 4));
  ...
  blackSoldierCols.forEach((col, index) => {
    pieces.push(
      createPiece(`black-soldier-${index + 1}`, "soldier", "black", 3, col),
    );
  });
  ...
  // Tướng
  pieces.push(createPiece("red-general", "general", "red", 9, 4));
  ...
  redSoldierCols.forEach((col, index) => {
    pieces.push(
      createPiece(`red-soldier-${index + 1}`, "soldier", "red", 6, col),
    );
  });
