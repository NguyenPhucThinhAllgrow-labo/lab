...
export function initialPositionHistory(
  board: ChineseChessPiece[],
  turn: PieceColor,
): PositionRecord[] {
  return [
...
function isProtected(
  board: ChineseChessPiece[],
  target: ChineseChessPiece,
): boolean {
  return board.some((defender) => {
    if (defender.color !== target.color || defender.id === target.id)
      return false;
    const probe = board.map((piece) =>
      piece.id === target.id
        ? {
            ...piece,
            color:
              defender.color === "red" ? ("black" as const) : ("red" as const),
          }
        : piece,
    );
    const probeDefender = probe.find((piece) => piece.id === defender.id)!;
    return isLegalMove(probe, probeDefender, {
      row: target.row,
      col: target.col,
    });
  });
...
function isPinned(
  ...
  return isInCheck(
    board.filter((piece) => piece.id !== target.id),
    target.color,
  );
}
...
export function chaseRelations(
  board: ChineseChessPiece[],
  color: PieceColor,
): ChineseChessChaseRelation[] {
  ...
  for (const attacker of board.filter((piece) => piece.color === color)) {
    for (const move of getPseudoLegalMoves(attacker, board)) {
      ...
      const kingOrPawnException =
        attacker.type === "general" || attacker.type === "soldier";
      const protectedRookException =
        target.type === "chariot" &&
        (attacker.type === "horse" || attacker.type === "cannon");
      const soldierMayBeChased =
        target.type !== "soldier" || crossedRiver(target);
...
export function recordPosition(
  ...
  history: PositionRecord[];
  state: ChineseChessRepetitionState;
  record: PositionRecord;
} {
  const records = history.length
    ? [...history]
    : initialPositionHistory(before, movedPiece.color);
  const beforeRelations = new Map(
    chaseRelations(before, movedPiece.color).map((item) => [
      `${item.attacker_id}>${item.target_id}`,
      item,
    ]),
  );
  const chases = chaseRelations(after, movedPiece.color).filter(
    (item) =>
      item.attacker_id === movedPiece.id ||
      !beforeRelations.has(`${item.attacker_id}>${item.target_id}`) ||
      (item.prohibited &&
        !beforeRelations.get(`${item.attacker_id}>${item.target_id}`)
          ?.prohibited),
  );
...
function classify(
  cycle: PositionRecord[],
  color: PieceColor,
): { kind: string; severity: number } {
  const moves = cycle.filter((record) => record.mover === color);
  if (!moves.length) return { kind: "none", severity: 0 };
  const prohibitedChases = (move: PositionRecord) =>
    move.chases.filter((chase) => chase.prohibited);
  const allCheck = moves.every((move) => move.gave_check);
  const allCoercive = moves.every(
    (move) => move.gave_check || prohibitedChases(move).length > 0,
  );
  ...
  if (allCoercive && hasCheck && hasChase)
    return { kind: "check_chase", severity: 2 };
...
export function adjudicateRepetition(
  ...
  const none: ChineseChessRepetitionState = {
    status: "none",
    count: 1,
  ...
  const occurrences = history
    .map((record, index) => (record.key === last.key ? index : -1))
