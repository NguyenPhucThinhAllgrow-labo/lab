...
export function generateBubbleSortSteps(input: number[]): SimulationStep[] {
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    for (let j = 0; j < n - i - 1; j++) {
      ...
      // So sánh
      addStep(`So sánh ${left} và ${right}.`, [j, j + 1]);
      ...
      if (left > right) {
        ...
        addStep(`Đã swap → ${values[j]!} đứng trước ${values[j + 1]!}.`);
      } else {
        // Không cần swap
        addStep(`${left} ≤ ${right} → giữ nguyên vị trí.`, [j, j + 1]);
      }
    }
    ...
    addStep(`${values[sortedIndex]!} đã ở đúng vị trí.`);
    ...
    if (!swapped) {
      ...
      addStep("Không có swap nào xảy ra → mảng đã được sắp xếp hoàn toàn.");
  ...
  addStep("Hoàn thành Bubble Sort. Tất cả phần tử đã được sắp xếp.");
