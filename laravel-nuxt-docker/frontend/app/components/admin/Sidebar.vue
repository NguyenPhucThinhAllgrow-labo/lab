...
const menuItems = [
  {
    label: "Lịch sử chơi cờ",
    to: "/admin/chinese-chess/history",
    icon: Trophy,
  },
  {
    label: "Dashboard",
...
<template>
  <aside
    class="fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-white/[0.06] bg-[#0d0d12] shadow-2xl shadow-black/20 transition-transform duration-300"
    :class="open ? 'translate-x-0' : '-translate-x-full'"
  >
    <!-- Header -->
    <div
      class="flex h-20 shrink-0 items-center border-b border-white/[0.06] px-6"
    >
      <!-- Logo -->
      <div
        class="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-violet-500 to-indigo-600 shadow-lg shadow-violet-500/20"
      >
      ...
      <div class="ml-3">
        ...
        <p class="text-[11px] text-zinc-600">Management System</p>
      </div>

      <!-- Close -->
      <button
        type="button"
        class="ml-auto flex h-8 w-8 items-center justify-center rounded-lg text-zinc-500 transition hover:bg-white/5 hover:text-white"
    ...
    <div class="flex-1 overflow-y-auto px-4 py-6">
      <nav class="space-y-1">
        ...
        <NuxtLink
          ...
          :key="item.to"
          :to="item.to"
          class="group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-zinc-500 transition-all duration-200 hover:bg-white/[0.04] hover:text-zinc-200"
        ...
        <div class="pt-1">
          ...
          >
            ...
            <ChevronDown
              class="ml-auto h-4 w-4 transition-transform duration-200"
              :class="algorithmMenuOpen ? 'rotate-180' : ''"
            />
          </button>

          <Transition name="algorithm-menu">
            <nav v-if="algorithmMenuOpen" class="mt-1 space-y-1 pl-6">
              <!-- Sorting -->
              <div>
                <button
                  type="button"
                  class="group flex w-full items-center gap-3 rounded-xl px-3 py-2 text-sm text-zinc-500 transition-all duration-200 hover:bg-white/[0.04] hover:text-zinc-200"
                  :aria-expanded="sortingMenuOpen"
                  @click="sortingMenuOpen = !sortingMenuOpen"
                >
                  <ArrowDownUp
                    class="h-4 w-4 shrink-0 transition-transform duration-200 group-hover:scale-105"
                  />
                  ...
                  <ChevronDown
                    class="ml-auto h-4 w-4 transition-transform duration-200"
                    :class="sortingMenuOpen ? 'rotate-180' : ''"
                  />
                </button>

                <!-- Sorting children -->
                <Transition name="sorting-menu">
                  <nav v-if="sortingMenuOpen" class="mt-1 space-y-1 pl-5">
                    <NuxtLink
    ...
    <div class="shrink-0 border-t border-white/[0.06] p-4">
      <!-- User Info -->
      <div
        class="flex items-center rounded-xl p-2 transition hover:bg-white/[0.04]"
      >
        <!-- Avatar -->
        <div
          class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-violet-500 to-indigo-600 text-xs font-bold text-white"
        >
        ...
        <div class="ml-3 min-w-0">
          ...
          <p class="truncate text-[11px] text-zinc-600">Administrator</p>
        </div>
      </div>
      ...
      <button
        type="button"
        class="mt-2 flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-zinc-500 transition-all duration-200 hover:bg-red-500/10 hover:text-red-400"
        aria-label="Logout"
        @click="handleLogout"
      >
