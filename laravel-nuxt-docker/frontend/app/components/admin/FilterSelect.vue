...
<template>
  ...
  >
    ...
    <Transition name="filter-menu">
      <div
        v-if="open"
        class="admin-filter-select__menu"
        role="listbox"
        :aria-label="ariaLabel"
      >
        ...
        >
          <span
            ...
            ><small>Hiển thị toàn bộ dữ liệu</small></span
          >
...
.admin-filter-select__trigger {
  ...
  border-radius: 11px;
  outline: 0;
  background: linear-gradient(180deg, #11111a, #0b0b12);
  color: #71717a;
  cursor: pointer;
...
.admin-filter-select__trigger:hover:not(:disabled),
.admin-filter-select.is-open .admin-filter-select__trigger {
  border-color: rgb(139 92 246 / 45%);
  background: #11111b;
  box-shadow: 0 0 0 3px rgb(139 92 246 / 8%);
}
.admin-filter-select__trigger:disabled {
  cursor: wait;
  opacity: 0.65;
}
.admin-filter-select__search,
.admin-filter-select__loading {
  width: 14px;
  height: 14px;
  flex: none;
}
...
.admin-filter-select__value strong {
  overflow: hidden;
  color: #e4e4e7;
  font-size: 11px;
  font-weight: 650;
...
.admin-filter-select__value small {
  color: #52525b;
...
.is-open .admin-filter-select__chevron {
...
.admin-filter-select__menu {
  ...
  overflow-y: auto;
  padding: 6px;
  border: 1px solid rgb(139 92 246 / 24%);
  border-radius: 12px;
  background: rgb(11 11 18 / 98%);
  box-shadow:
    0 18px 45px rgb(0 0 0 / 55%),
    0 0 0 1px rgb(255 255 255 / 2%);
  backdrop-filter: blur(16px);
}
.admin-filter-select__caption {
...
.admin-filter-select__option.is-selected {
...
.admin-filter-select__option > span {
  display: grid;
  min-width: 0;
  flex: 1;
  gap: 3px;
}
.admin-filter-select__option strong {
  overflow: hidden;
  color: #d4d4d8;
  font-size: 10px;
  font-weight: 650;
...
.admin-filter-select__option.is-selected strong {
  color: #ddd6fe;
}
.admin-filter-select__option small {
  color: #52525b;
  font-family: monospace;
  font-size: 8px;
}
.admin-filter-select__option svg {
  width: 13px;
  height: 13px;
  flex: none;
  color: #a78bfa;
