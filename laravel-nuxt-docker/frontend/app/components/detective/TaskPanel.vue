...
import type { Evidence, SupportedLocale, Task } from "~/types/games/detective";
...
function toggleTaskExplanation(task: Task) {
  explainedTaskId.value = explainedTaskId.value === task.id ? null : task.id;
}
...
function getDiscoveredCount(task: Task) {
...
<template>
  <section
    class="detective-running-frame rounded-lg border border-emerald-800/70 bg-[#030b08]/95 p-4 shadow-lg shadow-[0_0_30px_rgba(16,185,129,0.08)]"
    :class="[
      expanded && !grouped
  ...
  >
    ...
    <div class="mb-4 flex items-center justify-between">
      <div>
        <div
          class="font-mono text-xs uppercase tracking-[0.2em] text-emerald-300"
        >
      ...
      <div class="flex items-center gap-3">
        ...
        <button
          type="button"
          class="rounded border border-emerald-700/70 px-2 py-1 font-mono text-[9px] text-emerald-300 transition hover:bg-emerald-950/70"
    ...
    <div
      ...
      :class="expanded ? 'min-h-0 flex-1' : 'h-[420px]'"
    >
      <div class="space-y-3">
        <div
          v-if="activeTask"
        ...
        >
          <span class="relative flex h-2.5 w-2.5 shrink-0">
            <span
              class="absolute inline-flex h-full w-full animate-ping rounded-full bg-cyan-400 opacity-50"
            />
            <span
          ...
          <div
            class="font-mono text-[9px] uppercase tracking-[0.13em] text-cyan-300"
          >
            {{
              locale === "vi"
                ? "Đã nhận chỉ thị từ đơn vị điều tra"
                : "Assignment received from investigation unit"
            }}
          </div>
        </div>
        ...
        >
          <div class="flex gap-3">
            <button
              type="button"
              class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full border font-mono text-[10px]"
              :class="
              ...
              "
              :title="
                locale === 'vi' ? 'Giải thích nhiệm vụ' : 'Explain this task'
              "
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-start justify-between gap-3">
                <div
                  ...
                  :class="
                    activeTask.completed ? 'text-emerald-300' : 'text-stone-100'
                  "
                >
                  {{ getText(activeTask.title) }}
                  <span class="ml-1 font-mono text-cyan-500"
                    >[{{ activeTask.id }}]</span
                  >
                </div>

                <span class="shrink-0 font-mono text-[9px] text-amber-300">
              ...
              <div class="mt-1 text-[10px] leading-5 text-stone-300">
              ...
              <button
                ...
                type="button"
                class="mt-2 rounded border border-emerald-700/70 bg-emerald-950/35 px-2.5 py-1.5 font-mono text-[9px] uppercase tracking-[0.1em] text-emerald-300 transition hover:border-emerald-500 hover:bg-emerald-900/45"
              ...
              <div
                ...
                class="mt-3 rounded-md border border-amber-700/50 bg-amber-950/35 p-3"
              >
                <div
                  class="font-mono text-[9px] uppercase tracking-[0.14em] text-amber-300"
                >
                  {{
                    locale === "vi"
              ...
              <div class="mt-3 space-y-1.5 border-t border-stone-600/60 pt-3">
                <div
                    ...
                    {{
                      !linkingMode
                        ...
                        : selectedEvidence.length
                          ? locale === "vi"
                            ? `${selectedEvidence.length} evidence đã chọn`
                            : `${selectedEvidence.length} selected`
                          : locale === "vi"
                            ? "Chọn evidence ở panel [E]"
                            : "Select evidence in panel [E]"
                    }}
                  </span>
                </div>
                ...
                >
                  <span
                    class="flex h-4 w-4 shrink-0 items-center justify-center rounded-full border font-mono text-[8px]"
                    :class="
                      isEvidenceLinked(activeTask, item.id)
        ...
        >
          <div
            class="font-mono text-xs uppercase tracking-[0.16em] text-emerald-300"
          >
            {{
              locale === "vi"
                ? "Toàn bộ nhiệm vụ đã hoàn thành"
        ...
        >
          <div
            class="font-mono text-[9px] uppercase tracking-[0.12em] text-stone-500"
          >
            {{
              locale === "vi"
          ...
          <div class="mt-2 space-y-1 text-left">
            ...
            >
              [LOCKED] {{ locale === "vi" ? "CHỈ THỊ" : "ASSIGNMENT" }}
        ...
        <details
          ...
          class="rounded-md border border-stone-800 bg-black/20"
        >
          <summary
            class="cursor-pointer px-3 py-2.5 font-mono text-[9px] uppercase tracking-[0.12em] text-stone-500 hover:text-emerald-300"
          >
            {{
              locale === "vi"
                ? `Hồ sơ đã hoàn thành (${completedTasks.length})`
          ...
          <div class="space-y-2 border-t border-stone-800 p-2">
            ...
            >
              <summary
                class="completed-task-summary flex cursor-pointer list-none items-center justify-between gap-3 px-2.5 py-2 text-[10px] text-emerald-300 transition hover:bg-emerald-950/40"
              >
              ...
              <div class="border-t border-emerald-950/80 p-2.5">
                <div
                  class="mb-2 font-mono text-[9px] uppercase tracking-[0.12em] text-stone-500"
                >
                  {{
                    locale === "vi"
                ...
                >
                  ...
                  >
                    <span
                      class="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full border border-emerald-700 font-mono text-[8px] text-emerald-300"
                    >
                      ✓
                    </span>
                    <div class="min-w-0">
                ...
                <div v-else class="text-[9px] text-stone-600">
                  {{
                    locale === "vi"
                      ? "Không có evidence được liên kết."
                      : "No linked evidence."
                  }}
                </div>
    ...
    <div
      ...
      class="mt-4 shrink-0 border-t border-cyan-900/70 pt-4"
    >
      <div
        class="mb-2 text-center font-mono text-[9px] uppercase tracking-[0.12em] text-emerald-300"
      >
        {{
          locale === "vi"
      ...
      <button
        type="button"
        class="w-full rounded-md border border-cyan-500 bg-cyan-950/60 px-4 py-3 font-mono text-[11px] font-semibold uppercase tracking-[0.12em] text-cyan-100 shadow-lg shadow-cyan-950/40 transition hover:border-cyan-300 hover:bg-cyan-900/70"
