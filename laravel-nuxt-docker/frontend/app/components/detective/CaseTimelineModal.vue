  ...
  props.events
    ...
    .sort((left, right) => {
      const timeDifference =
        timeValue(left.event.time) - timeValue(right.event.time);
...
<template>
  <div
    class="fixed inset-0 z-[105] flex items-center justify-center bg-black/85 p-4 backdrop-blur-sm md:p-8"
    role="presentation"
    @click.self="emit('close')"
  >
    <section
      class="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-xl border border-cyan-800/70 bg-slate-950 shadow-[0_0_70px_rgba(6,182,212,0.15)]"
      role="dialog"
      aria-modal="true"
      aria-labelledby="case-timeline-title"
    >
      <header
        class="flex shrink-0 items-center justify-between gap-4 border-b border-cyan-900/70 bg-cyan-950/25 px-5 py-4"
      >
        <div>
          <p
            class="font-mono text-[10px] uppercase tracking-[0.25em] text-cyan-500"
          >
        ...
        <button
          type="button"
          class="rounded border border-cyan-800 px-3 py-1.5 font-mono text-xs text-cyan-300 transition hover:bg-cyan-950"
          @click="emit('close')"
        >
      ...
      <div
        class="shrink-0 border-b border-slate-800 bg-black/25 px-5 py-3 md:px-8"
      >
        <div
          class="flex items-center justify-between gap-4 font-mono text-[10px] uppercase tracking-wider"
        >
          <span class="text-cyan-400">
            {{
              locale === "vi" ? "Tiến độ tái dựng" : "Reconstruction progress"
            }}
            · {{ progressPercent }}%
          </span>
          <span class="text-slate-500">
            {{ remainingEvents }}
            {{ locale === "vi" ? "mốc chưa xác minh" : "unverified events" }}
          </span>
        </div>
      ...
      <div class="timeline-scrollbar overflow-y-auto px-5 py-6 md:px-8">
        <div
          v-if="!events.length"
          class="rounded-lg border border-dashed border-slate-700 bg-black/20 px-5 py-12 text-center"
        >
        ...
        <ol v-else class="relative ml-8 border-l border-slate-700/80">
          ...
          >
            <span
              class="absolute -left-[27px] top-0 flex h-14 w-14 items-center justify-center rounded-full border font-mono text-xs font-bold shadow-lg"
            ...
            <article class="rounded-lg border border-slate-800 bg-black/30 p-4">
              ...
              >
                {{
                  locale === "vi"
                    ? "Kết quả triển khai tác chiến"
                    : "Operational deployment result"
                }}
              </div>
            </article>
          </li>
      ...
      <footer
        class="flex shrink-0 flex-wrap gap-x-5 gap-y-2 border-t border-slate-800 px-5 py-3 font-mono text-[10px] text-slate-500"
      >
        <span
          ><i class="mr-1 inline-block h-2 w-2 rounded-full bg-slate-500" />{{
            locale === "vi" ? "Bối cảnh trước vụ án" : "Case background"
          }}</span
        >
        <span
          ><i class="mr-1 inline-block h-2 w-2 rounded-full bg-red-500" />{{
            locale === "vi" ? "Hành vi tại hiện trường" : "Incident action"
          }}</span
        >
        <span
          ><i class="mr-1 inline-block h-2 w-2 rounded-full bg-amber-500" />{{
            locale === "vi" ? "Di chuyển và dấu vết" : "Movement and trace"
          }}</span
        >
        <span
          ><i class="mr-1 inline-block h-2 w-2 rounded-full bg-emerald-500" />{{
            locale === "vi" ? "Ứng phó của cảnh sát" : "Police response"
          }}</span
        >
      </footer>
    </section>
