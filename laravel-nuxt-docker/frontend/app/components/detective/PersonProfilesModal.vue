...
import type { PersonProfile, SupportedLocale } from "~/types/games/detective";
...
const discoveredIds = computed(() => new Set(props.discoveredEvidenceIds));
...
<template>
  ...
  >
    ...
    >
      <header
        class="flex shrink-0 items-center justify-between gap-4 border-b border-violet-900/70 bg-violet-950/20 px-5 py-4"
      >
        <div>
          <p
            class="font-mono text-[10px] uppercase tracking-[0.25em] text-violet-400"
          >
            {{
          ...
          <h2
            id="person-profiles-title"
            class="mt-1 text-lg font-semibold text-violet-100"
          >
            {{
              locale === "vi"
      ...
      <div class="grid min-h-0 flex-1 md:grid-cols-[240px_minmax(0,1fr)]">
        <nav
          class="flex gap-2 overflow-x-auto border-b border-slate-800 p-3 md:flex-col md:overflow-y-auto md:border-b-0 md:border-r"
        >
          <button
          ...
          >
            ...
            <div class="mt-1 text-[10px] leading-4 text-violet-300">
              {{ text(person.role) }}
            </div>
          </button>
        </nav>
        ...
        <div v-if="selectedPerson" class="overflow-y-auto p-5 md:p-7">
          <div class="border-b border-slate-800 pb-5">
            <p
              class="font-mono text-[10px] uppercase tracking-[0.2em] text-violet-500"
            >
            ...
            <p class="mt-3 text-sm leading-6 text-slate-400">
              {{ text(selectedPerson.summary) }}
            </p>
          </div>

          <div class="mt-5 space-y-3">
            <template
              v-for="(detail, index) in selectedPerson.details"
              :key="index"
            >
              <div
              ...
              >
                <div
                  class="font-mono text-[10px] uppercase tracking-wider text-violet-400"
                >
                  {{ text(detail.label) }}
                </div>
              ...
              <div
                v-else
                class="rounded-lg border border-dashed border-slate-800 bg-black/10 p-4"
              >
                <div
