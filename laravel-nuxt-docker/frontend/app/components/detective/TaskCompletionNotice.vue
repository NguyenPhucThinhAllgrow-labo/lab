...
import type { SupportedLocale, Task } from "~/types/games/detective";
  ...
  >
    <div class="flex items-start justify-between gap-4">
      <div>
        ...
        <h2 class="mt-1 text-sm font-semibold text-emerald-100">
          {{ text(task.title) }}
        </h2>
      </div>
      <button
        type="button"
        class="font-mono text-xs text-slate-500 hover:text-white"
        @click="emit('close')"
      >
        ✕
      </button>
    ...
    <div
      class="mt-4 rounded-lg border border-emerald-900/70 bg-emerald-950/25 p-3"
    >
      <div
        class="font-mono text-[9px] uppercase tracking-wider text-emerald-500"
      >
