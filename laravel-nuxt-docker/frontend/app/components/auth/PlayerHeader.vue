...
const themeClass = computed(() => {
  ...
  if (route.path.startsWith("/games/chinese-chess"))
    return "player-header--chess";
  if (route.path.startsWith("/games/aim")) return "player-header--aim";
  if (route.path.startsWith("/games/reaction"))
    return "player-header--reaction";
...
onMounted(() => {
  try {
    collapsed.value =
      localStorage.getItem("player-header-collapsed") === "true";
  } catch {
...
<template>
  <div
    class="game-header-shell"
    :class="[themeClass, { 'is-collapsed': collapsed }]"
  >
    <header class="home-header">
      ...
      <nav class="home-nav" aria-label="Điều hướng chính">
        ...
        <NuxtLink :to="{ path: '/', hash: '#algorithm-lab' }"
          >Thuật toán</NuxtLink
        >
...
<style scoped>
.game-header-shell {
  --home-border: #ffffff14;
  --home-muted: #aab5b0;
  --home-accent: #d5f487;
  --header-bg: #101715;
  --header-glow: #d5f48712;
  --header-avatar-bg: #d5f48710;
  position: relative;
  z-index: 900;
  background: var(--header-bg);
  color: #f0f3ed;
  font-family: Inter, ui-sans-serif, system-ui, sans-serif;
  box-shadow: 0 8px 30px var(--header-glow);
  transition:
    background-color 0.2s ease,
    color 0.2s ease;
}
.player-header--pandora {
  --home-accent: #34d399;
  --home-muted: #78a995;
  --home-border: #34d39925;
  --header-bg: #020806;
  --header-glow: #10b9811a;
  --header-avatar-bg: #064e3b80;
}
.player-header--chess {
  --home-accent: #e2a84b;
  --home-muted: #aaa398;
  --home-border: #f59e0b22;
  --header-bg: #0b0a09;
  --header-glow: #d9770618;
  --header-avatar-bg: #78350f52;
}
.player-header--aim {
  --home-accent: #67e8f9;
  --home-muted: #9bb4c4;
  --home-border: #22d3ee22;
  --header-bg: #08121b;
  --header-glow: #06b6d41a;
  --header-avatar-bg: #164e6360;
}
.player-header--reaction {
  --home-accent: #fb7185;
  --home-muted: #b8a3aa;
  --home-border: #fb718525;
  --header-bg: #160b10;
  --header-glow: #e11d4818;
  --header-avatar-bg: #88133752;
}
.player-header--tetris {
  --home-accent: #c4b5fd;
  --home-muted: #aaa5bc;
  --home-border: #a78bfa25;
  --header-bg: #100d1b;
  --header-glow: #8b5cf61a;
  --header-avatar-bg: #4c1d9560;
}
.game-header-shell *,
.game-header-shell *::before,
.game-header-shell *::after {
  box-sizing: border-box;
}
.game-header-shell a {
  text-decoration: none;
}
.game-header-shell a:focus-visible,
.game-header-shell button:focus-visible {
...
.home-account__identity strong,
.home-account__identity small {
  max-width: 130px;
  overflow: hidden;
...
.home-account__logout:hover:not(:disabled) {
  border-color: var(--home-accent);
  color: var(--home-accent);
}
.home-account__logout:disabled {
  cursor: wait;
  opacity: 0.5;
}
.home-account__logout svg {
  width: 16px;
  height: 16px;
}
.home-header-toggle {
...
.game-header-shell.is-collapsed {
  height: 0;
...
@keyframes player-header-spin {
  to {
    transform: rotate(1turn);
  }
}
:global(html:not([data-site-theme="dark"])) .game-header-shell {
  --home-border: #00000018;
  --home-muted: #26302b;
  --header-bg: color-mix(in srgb, var(--home-accent) 8%, white);
  --header-glow: #24351c0b;
  --header-avatar-bg: color-mix(in srgb, var(--home-accent) 18%, white);
  background: var(--header-bg);
  color: #000;
}
:global(html:not([data-site-theme="dark"])) .home-header {
  background: #fff;
  border-color: #c9d3c3;
  box-shadow: 0 6px 24px #24351c08;
}
:global(html:not([data-site-theme="dark"]))
  .game-header-shell
  :where(a, button, span, small, strong, div) {
  color: #000;
}
:global(html:not([data-site-theme="dark"])) .home-brand > span {
  background: color-mix(in srgb, var(--home-accent) 62%, white);
}
:global(html:not([data-site-theme="dark"])) .home-account__avatar {
  border-color: color-mix(in srgb, var(--home-accent) 48%, #9ca3af);
  background: var(--header-avatar-bg);
}
