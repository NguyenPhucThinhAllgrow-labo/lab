import type { Scenario, SupportedLocale } from "~/types/games/detective";
...
export function useDetectiveApi() {
  ...
  async function getCase(caseId: string, locale: SupportedLocale) {
    const response = await api<DataResponse<DetectiveCaseDetail>>(
  ...
  async function listCases(locale: SupportedLocale) {
    const response = await api<DataResponse<DetectiveCaseMetadata[]>>(
      "/api/detective/cases",
      {
        query: { locale },
      },
  ...
  async function getScenario(caseId: string) {
    const detail = await getCase(caseId, "en");

    return detail.scenario;
  }

  async function getProgress(caseId: string) {
    const response = await api<DataResponse<DetectiveProgress | null>>(
  ...
  async function saveProgress(
    ...
    const response = await api<DataResponse<DetectiveProgress>>(
      `/api/detective/cases/${caseId}/progress`,
      {
        method: "PUT",
        body: payload,
  ...
  async function resetProgress(caseId: string) {
    await ensureCsrf();

    await api(`/api/detective/cases/${caseId}/progress`, { method: "DELETE" });
  }

  async function getCompletionHistory() {
    const response = await api<DataResponse<DetectiveCompletionHistory[]>>(
  ...
  async function getCompletionHistoryDetail(historyId: number) {
    const response = await api<DataResponse<DetectiveCompletionHistory>>(
