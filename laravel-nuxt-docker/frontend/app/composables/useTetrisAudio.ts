...
const MELODY = [
  440, 523.25, 659.25, 523.25, 392, 493.88, 587.33, 493.88, 349.23, 440, 523.25,
  440, 329.63, 392, 493.88, 392,
];
...
export function useTetrisAudio() {
  ...
  onMounted(() => {
    soundEnabled.value =
      localStorage.getItem("tetris-sound-enabled") !== "false";
  });
  ...
  async function playStart() {
    ...
    notes.forEach((note, index) =>
      scheduleEffect(() => tone(note, 0.13, 0.035), index * 70),
    );
  ...
  function playGameOver(stopBackgroundMusic = true) {
    ...
    notes.forEach((note, index) =>
      scheduleEffect(() => tone(note, 0.25, 0.035, "sawtooth"), index * 130),
    );
  }
