...
const passwordChecks = computed(() => ({
  ...
  matches:
    password.value !== "" && password.value === passwordConfirmation.value,
}));
...
async function handleRegister() {
  ...
  try {
    ...
    errorMessage.value =
      error?.data?.message || "Không thể tạo tài khoản. Vui lòng thử lại.";
  } finally {
    loading.value = false;
  }
}
...
<template>
  <main class="register-page">
    ...
    <div
      class="register-page__glow register-page__glow--left"
      aria-hidden="true"
    ></div>
    <div
      class="register-page__glow register-page__glow--right"
      aria-hidden="true"
    ></div>
    ...
    <section class="register-card">
      ...
      <header class="register-card__header">
        ...
        <div>
          ...
          <p>
            Một tài khoản để lưu tiến trình và thành tích trong tất cả game.
          </p>
        </div>
      </header>
      ...
      <form novalidate @submit.prevent="handleRegister">
        ...
        <div class="register-requirements" aria-live="polite">
          <span :class="{ 'is-valid': passwordChecks.length }"
            ><CheckCircle2 /> Ít nhất 8 ký tự</span
          >
          <span :class="{ 'is-valid': passwordChecks.matches }"
            ><CheckCircle2 /> Mật khẩu trùng khớp</span
          >
        </div>
      ...
      <footer class="register-card__footer">
        <span
          ><ShieldCheck /> Tài khoản được bảo vệ bằng phiên đăng nhập an
          toàn</span
        >
