...
interface BestResponse extends PaginatedResponse<LeaderboardEntry> {
  data: PaginatedResponse<LeaderboardEntry>["data"] & LeaderboardData;
}
...
const caseFilterOptions = computed(() =>
  (leaderboard.value?.cases ?? []).map((item) => ({
    value: item.id,
    label: item.title,
    meta: `[${item.id}]`,
  })),
...
const summaryCards = computed(() => [
  ...
  },
  {
    ...
    value: formatDuration(
      leaderboard.value?.summary.average_elapsed_seconds ?? 0,
    ),
...
const tableGroups = computed(() =>
  leaderboardGroups.value.map((group) => ({
    key: group.case.id,
    title: group.case.title,
    rows: group.entries,
  })),
);
...
async function loadBest(page = bestPagination.value.current_page) {
  ...
  errorMessage.value = "";
  try {
    const response = await api<BestResponse>(
      "/api/admin/pandora/leaderboard/best",
      {
        query: {
          case_id: selectedCase.value || undefined,
          locale: "vi",
          page,
          per_page: bestPagination.value.per_page,
        },
      },
    );
    ...
    if (request !== bestRequest) return;
    errorMessage.value =
      error?.data?.message || "Không thể tải thành tích tốt nhất.";
  } finally {
...
async function loadHistory(page = historyPagination.value.current_page) {
  ...
  historyError.value = "";
  try {
    const response = await api<PaginatedResponse<CompletionHistoryEntry>>(
      "/api/admin/pandora/leaderboard/history",
      {
        query: {
    ...
    if (request !== historyRequest) return;
    historyError.value =
      error?.data?.message || "Không thể tải lịch sử lượt chơi.";
  } finally {
  ...
  <main class="pandora-ranking">
    <header class="pandora-ranking__header">
      <div>
        ...
        <p>
          Dữ liệu tổng hợp trực tiếp từ các lượt hoàn thành được lưu trong hệ
          thống.
        </p>
      </div>
      ...
      <button
        type="button"
        class="pandora-ranking__refresh"
        :disabled="bestLoading || historyLoading"
        @click="refreshAll"
      >
    ...
    <section
      class="rounded-2xl mt-[18px] border border-white/[0.06] bg-[#11111b]"
    >
      <AdminFilterBar>
        <div>
      ...
      <AdminDataTable
        ...
        :row-key="(entry) => entry.history_id"
        :loading="bestLoading"
        :error="errorMessage"
        empty-text="Chưa có thành tích"
        empty-description="Người chơi hoàn thành case sẽ xuất hiện tại đây."
        @retry="loadBest()"
      >
          ...
          <td>
            <div class="ranking-player">
              ...
              <div>
                ...
                ><small>{{ entry.player.email || "Không có email" }}</small>
              </div>
            </div>
          </td>
          <td>
            <div class="ranking-score">
              ...
              ><span :class="`grade-${entry.grade.toLowerCase()}`">{{
                entry.grade
              }}</span>
            </div>
          </td>
          <td class="ranking-mono">
      ...
      <AdminPagination
        ...
        :loading="bestLoading"
        item-label="thành tích"
        @page-change="loadBest"
      />
    </section>

    <section
      class="pandora-ranking__history-panel rounded-2xl border border-white/[0.06] bg-[#11111b]"
      v-if="leaderboard"
    >
      <AdminFilterBar>
        <div>
          ...
          <p>
            Mỗi dòng là một lần hoàn thành riêng biệt, mới nhất được hiển thị
            trước.
          </p>
        </div>
      ...
      <AdminDataTable
        title="Lịch sử tất cả lượt chơi"
        :rows="historyEntries"
        :columns="[
          'Lượt',
          'Người chơi',
        ...
        :loading="historyLoading"
        :error="historyError"
        empty-text="Không tìm thấy lượt chơi phù hợp"
        empty-description="Thử thay đổi tên người chơi hoặc case đang lọc."
        @retry="loadHistory()"
      >
        <template #row="{ row: entry }">
          <td>
          ...
          <td>
            <strong class="ranking-case">{{ entry.case.title }}</strong
            ><small class="ranking-case-id">{{ entry.case.id }}</small>
          </td>
          <td>
            <div class="ranking-score">
              <strong>{{ entry.score }}</strong
