...
async function submitUser() {
  ...
  try {
    ...
    const response = await api<{ data?: AdminUser }>(
      `/api/admin/users${mode.value === "create" ? "" : `/${id}`}`,
      {
        method:
          mode.value === "delete"
            ? "DELETE"
    ...
    await loadUsers(
      mode.value === "create"
        ? 1
        : mode.value === "delete" && users.value.length === 1
...
async function loadUsers(page = pagination.value.current_page) {
  ...
  try {
    ...
    if (requestId !== requestSequence) return;
    errorMessage.value =
      error?.data?.message || "Không thể tải danh sách người dùng.";
  } finally {
  ...
  <main class="admin-users-page">
    <header class="admin-users-header">
      ...
      <div class="admin-users-actions">
        <button
          class="admin-users-button is-primary"
          type="button"
    ...
    <section class="rounded-2xl border border-white/[0.06] bg-[#11111b]">
      ...
      <AdminDataTable
        ...
        empty-description="Thử thay đổi từ khóa hoặc vai trò đang lọc."
        @retry="loadUsers(1)"
      >
        <template #row="{ row: item }">
          ...
          <td>
            <span
              class="admin-user-verification"
              :class="{ 'is-verified': item.email_verified_at }"
            >
              <UserRoundCheck />
          ...
          <td>
            <div class="admin-users-actions">
              <button
                class="admin-users-button is-icon"
                type="button"
              ...
              <button
                class="admin-users-button is-icon is-danger"
                type="button"
    ...
    <Teleport to="body">
      ...
      >
        <header>
          <h2 id="user-dialog-title">{{ dialogTitle }}</h2>
          <button
            type="button"
            class="admin-users-button is-icon"
            aria-label="Đóng"
        ...
        <form @submit.prevent="submitUser">
          <p v-if="mode === 'delete'" class="admin-users-delete-description">
            Xóa tài khoản <strong>{{ selectedUser?.name }}</strong> ({{
              selectedUser?.email
            }})? Người dùng sẽ không thể đăng nhập bằng tài khoản này nữa.
          </p>
          <fieldset v-else :disabled="saving">
            <label
              >Họ tên<input
                ...
                autocomplete="name"
                autofocus
              /><span v-if="fieldErrors.name" class="admin-users-form-error">{{
                fieldErrors.name[0]
              }}</span></label
            >
            <label
              >Email<input
                ...
                maxlength="255"
                autocomplete="email"
              /><span v-if="fieldErrors.email" class="admin-users-form-error">{{
                fieldErrors.email[0]
              }}</span></label
            >
            <label
              ...
              >
                <option value="user">Người chơi</option>
                <option value="admin">Quản trị viên</option></select
              ><span v-if="fieldErrors.role" class="admin-users-form-error">{{
                fieldErrors.role[0]
              }}</span></label
            >
            <label
          ...
          <p v-if="formError" class="admin-users-form-error" role="alert">
            {{ formError }}
          </p>
          <footer>
            <button
              type="button"
              class="admin-users-button"
            ...
            ><button
              type="submit"
              class="admin-users-button"
              :class="mode === 'delete' ? 'is-danger-solid' : 'is-primary'"
              :disabled="saving"
            >
              {{
                saving
