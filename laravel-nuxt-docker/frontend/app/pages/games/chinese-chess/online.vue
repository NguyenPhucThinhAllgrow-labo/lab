...
import type {
  ChineseChessPresenceMember,
  ChineseChessRoomEvent,
} from "~/composables/useChineseChessRealtime";
...
const gameStarted = computed(
  () => room.value?.status === "playing" || room.value?.status === "paused",
);
const spectatorMode = computed(
  () => room.value?.is_spectator || !room.value?.your_color,
);
const spectators = computed(() =>
  presenceMembers.value.filter(
    (member) =>
      member.id !== room.value?.red_player.id &&
      member.id !== room.value?.black_player?.id,
  ),
);
const isMyTurn = computed(
  () =>
    ...
    room.value.your_color === room.value.current_turn,
);
const displayedBoard = computed(
  () => pendingMove.value?.position ?? room.value?.board ?? [],
);
const displayedCurrentTurn = computed<PieceColor>(() =>
  pendingMove.value
    ? pendingMove.value.color === "red"
      ? "black"
      : "red"
...
const isReady = computed(() => {
  ...
  return room.value.your_color === "red"
    ? room.value.red_ready
    : room.value.black_ready;
});
...
const redCaptured = computed(
  () =>
    room.value?.move_history
      .filter((move) => move.color === "red" && move.captured)
      .map((move) => move.captured!) ?? [],
);
const blackCaptured = computed(
  () =>
    room.value?.move_history
      .filter((move) => move.color === "black" && move.captured)
      .map((move) => move.captured!) ?? [],
);
const lastMove = computed(
  () => pendingMove.value ?? room.value?.move_history.at(-1) ?? null,
);
const rematchRequested = computed(() =>
  room.value?.your_color === "red"
    ? room.value.red_rematch
    : room.value?.black_rematch,
);
const myUndosRemaining = computed(() =>
  room.value?.your_color === "black"
    ? room.value.black_undos_remaining
    : (room.value?.red_undos_remaining ?? 0),
);
const isUndoRequester = computed(
  () =>
    !!room.value?.undo_request &&
    room.value.undo_request.color === room.value.your_color,
);
const mustRespondToUndo = computed(
  () =>
    !spectatorMode.value &&
    !!room.value?.undo_request &&
    !isUndoRequester.value,
);
...
const finishReasonText = computed(() => {
  const reasons: Record<string, string> = {
    ...
    timeout: "Hết giờ",
    surrender: "Đầu hàng",
    player_left: "Đối thủ rời phòng",
    host_left: "Chủ phòng đã rời đi",
    perpetual_check: "Thua do chiếu liên tục",
    perpetual_chase: "Thua do đuổi quân liên tục",
    perpetual_check_chase: "Thua do chiếu và đuổi quân liên tục",
    repetition_draw: "Hòa do lặp lại thế cờ",
  };
  return reasons[room.value?.finish_reason ?? ""] ?? "";
});
const hasWinner = computed(
  () => room.value?.status === "finished" && !!room.value.winner,
);
const playerLeft = computed(() =>
  ["player_left", "host_left"].includes(room.value?.finish_reason ?? ""),
);
...
const statusText = computed(() => {
  ...
  if (spectatorMode.value) {
    ...
    if (room.value.status === "playing")
      return `Đang xem · Lượt quân ${room.value.current_turn === "red" ? "Đỏ" : "Đen"}`;
  ...
  if (room.value.status === "playing")
    return isMyTurn.value ? "Đến lượt bạn" : "Đang chờ đối thủ đi";
  if (room.value.status === "paused")
    return `${room.value.paused_by?.name ?? "Một người chơi"} đã tạm dừng`;
...
function handleDeletedRoom(id: number): void {
  ...
  resetLobby();
  errorMessage.value =
    "Ván đấu đã bị quản trị viên xóa. Bạn đã được đưa ra khỏi phòng.";
}
...
function applyRoom(next: ChineseChessRoom, keepColor = false): void {
  ...
  const ownColor = keepColor
    ? (room.value?.your_color ?? null)
    : next.your_color;
  ...
  if (
    ...
    [
      ...
      "repetition_draw",
    ].includes(updatedRoom.finish_reason ?? "") &&
    ...
    if (updatedRoom.finish_reason === "repetition_draw") {
      surrenderResult.value = {
        title: "Ván đấu hòa",
        message:
          "Thế cờ đã xuất hiện lần thứ ba và không bên nào có mức vi phạm cao hơn.",
        won: false,
      };
      return;
    }
    ...
    const winnerColor: PieceColor =
      updatedRoom.winner?.id === updatedRoom.red_player.id ? "red" : "black";
    const won = updatedRoom.your_color === winnerColor;
    const loser =
      winnerColor === "red" ? updatedRoom.black_player : updatedRoom.red_player;
...
function messageFrom(error: any): string {
  ...
  if (errors)
    return String(Object.values(errors).flat()[0] ?? "Yêu cầu không hợp lệ.");
  return error?.data?.message ?? error?.message ?? "Không thể kết nối máy chủ.";
}

async function createRoom(): Promise<void> {
  await request(async () => {
    const response = await api<{ room: ChineseChessRoom }>(
...
async function joinRoom(): Promise<void> {
  ...
  await request(async () => {
    const response = await api<{ room: ChineseChessRoom }>(
      `/api/chinese-chess/rooms/${code}/join`,
      { method: "POST" },
    );
    applyRoom(response.room);
    await enterRoom(response.room);
...
function applyRealtimeMove(event: ChineseChessRoomEvent): boolean {
  ...
  if (event.state.move_count !== room.value.move_history.length + 1)
    return false;
...
async function syncRoomFromRealtime(
  event: ChineseChessRoomEvent,
): Promise<void> {
  if (
    !room.value ||
    event.room_id !== room.value.id ||
    event.version <= room.value.version
  )
    return;
...
async function submitMove(move: ChineseChessMoveHistory): Promise<void> {
  ...
  try {
    await request(async () => {
      const response = await api<{ room: ChineseChessRoom }>(
        `/api/chinese-chess/rooms/${code}/moves`,
        {
          method: "POST",
...
async function markReady(): Promise<void> {
  if (
    ...
    !room.value.black_player ||
    isReady.value
  )
    return;
  await request(async () => {
    const response = await api<{ room: ChineseChessRoom }>(
      `/api/chinese-chess/rooms/${room.value!.code}/ready`,
      { method: "POST" },
    );
    applyRoom(response.room);
  });
...
async function undoLastMove(): Promise<void> {
  ...
  await request(async () => {
    const response = await api<{ room: ChineseChessRoom }>(
      `/api/chinese-chess/rooms/${room.value!.code}/undo`,
      {
        method: "POST",
...
async function respondToUndo(accepted: boolean): Promise<void> {
  ...
  await request(async () => {
    const response = await api<{ room: ChineseChessRoom }>(
      `/api/chinese-chess/rooms/${room.value!.code}/undo/respond`,
      {
        method: "POST",
...
async function leaveRoom(): Promise<void> {
  ...
  if (
    ["playing", "paused"].includes(room.value.status) &&
    !confirm("Rời phòng khi đang chơi sẽ bị xử thua. Tiếp tục?")
  )
    return;
  await request(async () => {
    await api(`/api/chinese-chess/rooms/${room.value!.code}/leave`, {
      method: "POST",
    });
    resetLobby();
  });
...
async function roomAction(
  action: "surrender" | "rematch" | "pause" | "resume",
): Promise<void> {
  if (!room.value) return;
  await request(async () => {
    const response = await api<{ room: ChineseChessRoom }>(
      `/api/chinese-chess/rooms/${room.value!.code}/${action}`,
      { method: "POST" },
    );
    applyRoom(response.room);
  });
...
function displayedTime(color: PieceColor): number {
  ...
  const base =
    color === "red"
      ? room.value.red_time_seconds
      : room.value.black_time_seconds;
  if (room.value.status !== "playing" || room.value.current_turn !== color)
    return base;
...
function positionText(
  move: Pick<ChineseChessMoveHistory, "from" | "to">,
): string {
...
watch(clockTick, async () => {
  if (!room.value || room.value.status !== "playing" || timeoutSyncing.value)
    return;
...
onMounted(async () => {
  ...
  const code =
    typeof route.query.room === "string" ? route.query.room.toUpperCase() : "";
  if (code) {
    roomCode.value = code;
    await request(async () => {
      const response = await api<{ room: ChineseChessRoom }>(
...
<template>
  <main
    class="online-chess"
    :class="{ 'has-active-room': room, 'is-resizing': resizingViewport }"
  >
    ...
    <header class="online-chess__header">
      ...
      <div
        class="online-chess__connection"
        :class="{ 'is-online': realtimeConnected }"
      >
    ...
    <section v-if="!room" class="chess-lobby">
      <div class="chess-lobby__intro">
        ...
        <p>
          Tạo phòng riêng hoặc nhập mã được chia sẻ. Mỗi nước đi được xác thực
          và đồng bộ bởi máy chủ.
        </p>
        <ul>
      ...
      <div class="chess-lobby__actions">
        <button
          class="chess-button chess-button--primary"
          :disabled="busy"
          @click="createRoom"
        >
        ...
        <div class="chess-lobby__join">
          <input
            ...
            v-model="roomCode"
            maxlength="8"
            placeholder="VD: XQ7K2P"
            @keyup.enter="joinRoom"
          />
          <button class="chess-button" :disabled="busy" @click="joinRoom">
            Vào phòng
          </button>
        </div>
    ...
    <template v-else>
      <p v-if="errorMessage" class="chess-error chess-error--room">
        {{ errorMessage }}
      </p>
      ...
      <section class="online-match">
        <nav class="mobile-chess-toolbar" aria-label="Thông tin ván đấu">
          <div>
            ...
            <small
              >Đen {{ timeText("black") }} · Đỏ {{ timeText("red") }}</small
            >
          </div>
          <button
            type="button"
            :class="{ 'is-active': mobilePanel === 'history' }"
            @click="toggleMobilePanel('history')"
          >
            Lịch sử <b>{{ room.move_history.length }}</b>
          </button>
          <button
            type="button"
            :class="{ 'is-active': mobilePanel === 'players' }"
            @click="toggleMobilePanel('players')"
          >
            Trận đấu
          </button>
        </nav>
        ...
        <aside
          class="match-panel match-panel--history"
          :class="{ 'is-mobile-open': mobilePanel === 'history' }"
        >
          <div class="match-panel__title">
            <span>Lịch sử nước đi</span><b>{{ room.move_history.length }}</b
            ><button
              type="button"
          ...
          <div class="move-list">
            <p v-if="!room.move_history.length" class="match-empty">
              Chưa có nước đi nào
            </p>
            <div
              v-for="move in room.move_history"
              :key="move.number"
              class="move-item"
            >
              ...
              <strong
                class="move-piece-token"
                :class="`is-${move.piece.color}`"
                >{{ pieceName(move.piece) }}</strong
              >
              ...
              <span class="move-item__result">
                <template v-if="move.captured">
                  ...
                  <strong
                    class="move-piece-token is-captured"
                    :class="`is-${move.captured.color}`"
                    >{{ pieceName(move.captured) }}</strong
                  >
                </template>
                <strong
                  v-if="move.is_check"
                  class="move-check-badge"
                  title="Nước đi này chiếu tướng"
                  >Chiếu</strong
                >
                <strong
                  v-if="
                    move.rule_action === 'chase' ||
                    move.rule_action === 'check_chase'
                  "
                  class="move-chase-badge"
                  title="Nước đi này tạo truy đuổi"
                  >Tróc</strong
                >
              </span>
            </div>
          ...
          <div class="captured-block">
            ...
            <div class="captured-row">
              <span>Đỏ</span>
              <div class="captured-piece-list">
                <b
                  v-for="piece in redCaptured"
                  :key="piece.id"
                  class="captured-piece-token"
                  :class="`is-${piece.color}`"
                  >{{ pieceName(piece) }}</b
                ><small v-if="!redCaptured.length">—</small>
              </div>
            </div>
            <div class="captured-row">
              <span>Đen</span>
              <div class="captured-piece-list">
                <b
                  v-for="piece in blackCaptured"
                  :key="piece.id"
                  class="captured-piece-token"
                  :class="`is-${piece.color}`"
                  >{{ pieceName(piece) }}</b
                ><small v-if="!blackCaptured.length">—</small>
              </div>
            </div>
          </div>
        </aside>
        ...
        <section class="online-board-wrap">
          ...
          <div v-if="room.status === 'waiting'" class="board-overlay">
            <div class="board-waiting-card" aria-live="polite">
              <div
                class="board-waiting-card__icon"
                :class="{ 'is-ready': isReady }"
              >
                <Eye v-if="spectatorMode" :size="24" />
                <LoaderCircle
                  v-else-if="!room.black_player"
                  class="animate-spin"
                  :size="24"
                />
              ...
              <span class="board-waiting-card__eyebrow"
                >PHÒNG {{ room.code }}</span
              >
              <h2>
                {{
                  spectatorMode
              ...
              <p v-if="spectatorMode">
                Ván đấu sẽ tự động hiển thị khi hai người chơi bắt đầu.
              </p>
              <p v-else-if="!room.black_player">
                Chia sẻ mã phòng để mời thêm một người chơi.
              </p>
              <p v-else-if="isReady">
                Ván đấu tự động bắt đầu ngay khi đối thủ sẵn sàng.
              </p>
              <p v-else>
                {{
                  opponentReady
                    ? "Đối thủ đã sẵn sàng và đang chờ bạn."
                    : "Xác nhận khi bạn đã sẵn sàng bắt đầu."
                }}
              </p>

              <div v-if="room.black_player" class="board-waiting-card__players">
                <span :class="{ 'is-ready': room.red_ready }"
                  ><i></i> Quân Đỏ</span
                >
                <b>VS</b>
                <span :class="{ 'is-ready': room.black_ready }"
                  ><i></i> Quân Đen</span
                >
              </div>

              <button
                v-if="!spectatorMode && room.black_player"
                class="chess-button chess-button--ready"
                :disabled="busy || isReady"
                @click="markReady"
              >
                <LoaderCircle v-if="busy" class="animate-spin" :size="17" />
                <Check v-else :size="17" />
                {{ isReady ? "Đã sẵn sàng" : "Tôi đã sẵn sàng" }}
              </button>
            </div>
          </div>
          <div
            v-else-if="room.status === 'paused'"
            class="board-overlay board-overlay--paused"
          >
            <div class="board-waiting-card pause-card" aria-live="polite">
              <div class="board-waiting-card__icon is-paused">
                <Pause :size="24" />
              </div>
              <span class="board-waiting-card__eyebrow"
                >VÁN ĐẤU ĐANG TẠM DỪNG</span
              >
              <h2>Đồng hồ đã dừng</h2>
              <p>
                <strong>{{ room.paused_by?.name ?? "Một người chơi" }}</strong>
                đã tạm dừng ván đấu.
              </p>
              <button
                v-if="!spectatorMode"
                class="chess-button chess-button--resume"
                :disabled="busy"
                @click="togglePause"
              >
                <LoaderCircle v-if="busy" class="animate-spin" :size="17" />
        ...
        <div
          class="match-player-column"
          :class="{ 'is-mobile-open': mobilePanel === 'players' }"
        >
          ...
          <section class="chess-roombar chess-roombar--sidebar">
            <div class="chess-roombar__room">
              ...
              <button
                class="chess-roombar__code"
                title="Sao chép mã phòng"
                @click="copyCode"
              >
                {{ room.code }} <Check v-if="copied" :size="15" /><Copy
                  v-else
                  :size="15"
                />
              </button>
            ...
            <div class="chess-roombar__match">
              <div class="chess-round">
                ...
                <small
                  >{{
                    room.starting_color === "red" ? "Quân Đỏ" : "Quân Đen"
                  }}
                  đi trước</small
                >
              </div>
              <div class="chess-roombar__status" :class="`is-${room.status}`">
                <span></span>
                <div v-if="hasWinner" class="chess-result-text">
                  <strong class="chess-result-winner"
                    >{{ room.winner?.name }} thắng</strong
                  >
                  <em :class="{ 'chess-result-left': playerLeft }">{{
                    finishReasonText || "Kết thúc"
                  }}</em>
                </div>
                <div v-else :class="{ 'chess-result-left': playerLeft }">
                  {{ statusText }}
                </div>
              </div>
            </div>
            <button
              class="chess-button chess-button--danger-ghost chess-roombar__leave"
              :disabled="busy"
              @click="leaveRoom"
            >
          ...
          <aside class="match-panel match-panel--players">
            <div class="match-panel__title">
              <span>Người chơi</span><Users :size="17" />
            </div>
            <div
            ...
            >
              <span
                class="player-turn__dot"
                :class="{ 'is-red': room.current_turn === 'red' }"
              ></span>
            ...
            <div
              ...
              :class="{
                'is-active':
                  room.status === 'playing' && room.current_turn === 'black',
              }"
            >
              <div class="player-avatar is-black">將</div>
              <div>
                <span>QUÂN ĐEN</span
                ><strong>{{ room.black_player?.name ?? "Đang chờ..." }}</strong
                ><em
                  v-if="room.status === 'waiting' && room.black_player"
                  :class="{ 'is-ready': room.black_ready }"
                  >{{ room.black_ready ? "Đã sẵn sàng" : "Chưa sẵn sàng" }}</em
                >
              </div>
            ...
            <div
              ...
              :class="{
                'is-active':
                  room.status === 'playing' && room.current_turn === 'red',
              }"
            >
              <div class="player-avatar is-red">帥</div>
              <div>
                <span>QUÂN ĐỎ</span><strong>{{ room.red_player.name }}</strong
                ><em
                  v-if="room.status === 'waiting'"
                  :class="{ 'is-ready': room.red_ready }"
                  >{{ room.red_ready ? "Đã sẵn sàng" : "Chưa sẵn sàng" }}</em
                >
              </div>
            ...
            <div class="match-notice" :class="{ 'is-mine': isMyTurn }">
              ...
              <div>
                <template v-if="hasWinner">
                  ...
                  >
                  <span
                    class="chess-result-reason"
                    :class="{ 'chess-result-left': playerLeft }"
                    >{{ finishReasonText || "Kết thúc" }}</span
                  >
                </template>
                <strong v-else :class="{ 'chess-result-left': playerLeft }">{{
                  statusText
                }}</strong>
                <span v-if="checkedColor"
                  >Tướng {{ checkedColor === "red" ? "Đỏ" : "Đen" }} đang bị
                  chiếu</span
                >
              </div>
            </div>
            ...
            <div class="match-actions">
              <div
                v-if="room.repetition?.status === 'warning'"
                class="repetition-warning"
                role="alert"
              >
                <strong>{{
                  room.repetition.obligated_color
                    ? `${room.repetition.obligated_color === room.your_color ? "Bạn" : "Đối thủ"} phải phá lặp`
                    : "Thế cờ đã lặp lần hai"
                }}</strong>
                <span>{{
                  room.repetition.obligated_color
                    ? "Hãy đổi nước để phá chuỗi lặp. Ván đấu không tự động xử thua."
                    : "Lặp lần ba sẽ xử hòa."
                }}</span>
              </div>
              <div
                v-if="!spectatorMode && isUndoRequester"
                class="undo-request-pending"
                role="status"
              >
              ...
              <button
                v-if="!spectatorMode && room.status === 'playing'"
                class="chess-button chess-button--pause"
                :disabled="busy || !!room.undo_request"
                @click="togglePause"
              >
                <Pause :size="17" /> Tạm dừng
              </button>
              <button
                v-if="!spectatorMode && room.status === 'paused'"
                class="chess-button chess-button--resume"
                :disabled="busy"
                @click="togglePause"
              >
              ...
              <button
                v-if="!spectatorMode && room.status === 'playing'"
                class="chess-button chess-button--undo"
                :disabled="busy || !canUndo"
                @click="undoLastMove"
              >
                <LoaderCircle v-if="busy" class="animate-spin" :size="17" />
                <Undo2 v-else :size="17" />
                {{ isUndoRequester ? "Đang chờ xác nhận" : "Xin đi lại" }}
                <span>({{ myUndosRemaining }}/3)</span>
              </button>
              <button
                v-if="
                  !spectatorMode &&
                ...
                "
                class="chess-button chess-button--danger-ghost"
                :disabled="busy"
                @click="surrender"
              >
                Đầu hàng
              ...
              <button
                v-if="!spectatorMode && room.status === 'finished'"
                class="chess-button chess-button--primary"
                :disabled="busy || rematchRequested"
                @click="requestRematch"
              >
                <RefreshCw :size="17" />
                {{ rematchRequested ? "Đang chờ đối thủ" : "Sẵn sàng" }}
              </button>
            </div>
            ...
            <section class="spectator-list" aria-live="polite">
              <button
                type="button"
                class="spectator-list__trigger"
                aria-haspopup="true"
              >
                <span><Eye :size="15" /> Người xem</span
                ><b>{{ spectators.length }}</b>
              </button>
              <div class="spectator-list__popover">
                <div v-if="spectators.length" class="spectator-list__members">
                  ...
                  >
                    <i>{{ viewer.name.slice(0, 1).toUpperCase() }}</i
                    >{{ viewer.name }}
                  </span>
                </div>
                <p v-else>
                  {{
                    realtimeConnected
                      ? "Chưa có người xem khác"
                      : "Danh sách sẽ hiện khi realtime kết nối"
                  }}
                </p>
              </div>
            </section>
    ...
    <Teleport to="body">
      <Transition name="chess-dialog">
        <div v-if="mustRespondToUndo && room" class="chess-dialog-backdrop">
          <section
            class="chess-dialog chess-dialog--undo"
            role="alertdialog"
            aria-modal="true"
            aria-labelledby="undo-dialog-title"
            aria-describedby="undo-dialog-description"
          >
            <div class="chess-dialog__icon is-warning">
              <Undo2 :size="28" />
            </div>
            ...
            <p id="undo-dialog-description">
              <strong>{{
                room.undo_request?.requester_name ?? "Đối thủ"
              }}</strong>
              xin đi lại. Đồng hồ đang tạm dừng trong lúc chờ bạn phản hồi.
            </p>
            <div class="chess-dialog__actions">
              <button
                class="chess-button"
                :disabled="busy"
                @click="respondToUndo(false)"
              >
                Từ chối
              </button>
              <button
      ...
      <Transition name="chess-dialog">
        <div
          v-if="surrenderConfirmOpen"
          class="chess-dialog-backdrop"
          role="presentation"
          @click.self="surrenderConfirmOpen = false"
        >
          <section
            class="chess-dialog"
            role="dialog"
          ...
          >
            <button
              class="chess-dialog__close"
              aria-label="Đóng"
              @click="surrenderConfirmOpen = false"
            >
              <X :size="18" />
            </button>
            <div class="chess-dialog__icon is-warning">
            ...
            <p>
              Sau khi xác nhận, đối thủ sẽ được xử thắng và hành động này không
              thể hoàn tác.
            </p>
            <div class="chess-dialog__actions">
              <button
              ...
              <button
                class="chess-button chess-button--danger"
                :disabled="busy"
      ...
      <Transition name="chess-dialog">
        ...
        >
          <section
            ...
            role="alertdialog"
            aria-modal="true"
            aria-labelledby="online-repetition-title"
          >
            <button
              class="chess-dialog__close"
              aria-label="Đóng"
              @click="repetitionBlockedMessage = ''"
            >
              <X :size="18" />
            </button>
            <div class="chess-dialog__icon is-warning">
      ...
      <Transition name="chess-dialog">
        <div
          v-if="surrenderResult"
          class="chess-dialog-backdrop"
          role="presentation"
        >
          <section
            class="chess-dialog chess-dialog--result"
            :class="{ 'is-winner': surrenderResult.won }"
            role="alertdialog"
            aria-modal="true"
            aria-labelledby="surrender-result-title"
          >
            <button
              class="chess-dialog__close"
              aria-label="Đóng"
              @click="surrenderResult = null"
            >
              <X :size="18" />
            </button>
            <div
              class="chess-dialog__icon"
              :class="surrenderResult.won ? 'is-winner' : 'is-finished'"
            >
              <Trophy v-if="surrenderResult.won" :size="30" />
              <Flag v-else :size="28" />
            </div>
            <span class="chess-dialog__eyebrow">{{
              surrenderResult.eyebrow ?? "VÁN ĐẤU KẾT THÚC"
            }}</span>
...
<style
  scoped
  src="~/assets/css/pages/games/chinese-chess/history-pieces.css"
></style>
<style scoped src="~/assets/css/pages/games/chinese-chess/ready.css"></style>
<style
  scoped
  src="~/assets/css/pages/games/chinese-chess/repetition.css"
></style>
<style scoped src="~/assets/css/pages/games/chinese-chess/dialogs.css"></style>
<style
  scoped
  src="~/assets/css/pages/games/chinese-chess/typography.css"
></style>
<style scoped src="~/assets/css/pages/games/chinese-chess/rounds.css"></style>

<style scoped>
.chess-result-winner,
.chess-dialog.is-winner h2 {
  color: #6ee7b7;
}
.chess-result-text em {
  font-style: normal;
}
.match-notice .chess-result-reason {
  color: #b2aca2;
}
.chess-result-left,
.match-notice .chess-result-left {
  color: #f87171;
}
...
<style
  scoped
  src="~/assets/css/pages/games/chinese-chess/player-turn.css"
></style>
<style
  scoped
  src="~/assets/css/pages/games/chinese-chess/spectators.css"
></style>
...
.online-chess.is-resizing :deep(.chess-board-frame::after) {
  filter: none;
}
.online-chess.is-resizing :deep(.chess-piece),
.online-chess.is-resizing :deep(.chess-piece::after) {
  filter: none;
}
.online-chess.is-resizing :deep(.board-overlay),
.online-chess.is-resizing .online-advisor-toast {
  backdrop-filter: none;
}
.online-chess.is-resizing :deep(.chess-piece) {
  box-shadow:
    0 2px 0 #75451f,
    inset 0 1px 2px rgb(255 249 215 / 50%);
}
