...
const games = [
  ...
  },
  {
    ...
    code: "XQ-02",
    description: "Xiangqi strategy board",
  },
  {
    name: "Reaction Test",
    path: "/games/reaction",
...
const chineseChessPieces = [
  ...
  { symbol: "車", side: "black", x: "14%", y: "39%", size: 45, delay: 0.45 },
  { symbol: "炮", side: "red", x: "4%", y: "72%", size: 50, delay: 0.9 },
  { symbol: "馬", side: "black", x: "25%", y: "85%", size: 38, delay: 1.35 },
  { symbol: "將", side: "black", x: "91%", y: "14%", size: 58, delay: 0.2 },
  { symbol: "象", side: "red", x: "83%", y: "37%", size: 43, delay: 0.65 },
  { symbol: "卒", side: "black", x: "93%", y: "69%", size: 48, delay: 1.1 },
  { symbol: "兵", side: "red", x: "76%", y: "87%", size: 36, delay: 1.55 },
  { symbol: "仕", side: "red", x: "18%", y: "8%", size: 40, delay: 0.3 },
  { symbol: "士", side: "black", x: "72%", y: "7%", size: 42, delay: 0.55 },
  { symbol: "相", side: "red", x: "3%", y: "46%", size: 44, delay: 0.75 },
  ...
  { symbol: "卒", side: "black", x: "39%", y: "5%", size: 34, delay: 0.85 },
...
const selectedGamePath = ref(safeGamePath(route.query.redirect));

const selectedGame = computed(
  () =>
    games.find((game) => selectedGamePath.value.startsWith(game.path)) ??
    games[0],
);
...
const loginTheme = computed(() => {
  if (isPandoraSelected.value) {
    return {
      ...
      description:
        "Phiên Pandora sẽ lưu tiến trình của bạn trên hệ thống bảo mật.",
  ...
  if (isTetrisSelected.value) {
    return {
      ...
      description:
        "Đăng nhập để lưu điểm số và tiếp tục hành trình xếp khối của bạn.",
  ...
  if (isChineseChessSelected.value) {
    return {
      ...
      description:
        "Đăng nhập để bắt đầu ván Chinese Chess và lưu lại hành trình thi đấu.",
...
function isHackerBitBright(column: number, bit: number) {
  const value =
    Math.imul(
      hackerBitSeed.value ^
        Math.imul(column + 11, 2246822519) ^
        Math.imul(bit + 7, 3266489917),
      1597334677,
    ) >>> 0;
...
watch(
  ...
  (isSelected) => {
    ...
    hackerBitTimer = setInterval(() => {
      hackerBitSeed.value =
        (Math.imul(hackerBitSeed.value, 1664525) + 1013904223) >>> 0;
    }, 260);
  },
  { immediate: true },
);
...
const handleLogin = async () => {
  ...
  try {
    ...
    error.value = err?.data?.message || "Đăng nhập thất bại. Vui lòng thử lại.";
  } finally {
    loading.value = false;
  }
};
  ...
  >
    ...
    <div class="game-login-backdrop" aria-hidden="true">
      <div v-if="isPandoraSelected" class="game-login-hacker-decor">
        ...
        <code class="hacker-coordinate hacker-coordinate--top"
          >NODE://PANDORA_GATE_01</code
        >
        <code class="hacker-coordinate hacker-coordinate--bottom"
          >AES-256 // SESSION ISOLATED</code
        >
    ...
    <Transition :name="loginBuildTransition" mode="out-in" appear>
      <section :key="selectedGame.path" class="game-login-shell">
        <div
          v-if="isPandoraSelected"
          class="hacker-border-trace"
          aria-hidden="true"
        >
        ...
        <div
          v-if="isTetrisSelected"
          class="tetris-build-blocks"
          aria-hidden="true"
        >
          <i
            v-for="block in 14"
        ...
        <aside class="game-login-intro">
          ...
          <nav class="game-login-list" aria-label="Game selection">
            ...
            >
              ...
              <span class="game-login-game"
                ><b>{{ game.name }}</b
                ><small>{{ game.description }}</small></span
              >
        ...
        <section class="game-login-form">
          ...
          <form novalidate @submit.prevent="handleLogin">
            ...
            <p v-if="error" class="game-login-error" role="alert">
              {{ error }}
            </p>
            ...
            <button class="game-login-submit" type="submit" :disabled="loading">
              {{
                loading
                  ? "ĐANG XÁC THỰC..."
                  : `VÀO ${selectedGame.name.toUpperCase()}`
              }}
              <span>→</span>
            </button>
          </form>
          ...
          <div class="game-login-account-links">
            <p class="game-login-register">
              Chưa có tài khoản?
              <NuxtLink to="/register">Đăng ký ngay →</NuxtLink>
            </p>
            <p class="game-login-admin">
              Quản trị viên?
              <NuxtLink to="/admin/login">Đăng nhập admin →</NuxtLink>
            </p>
          </div>
        </section>
      </section>
