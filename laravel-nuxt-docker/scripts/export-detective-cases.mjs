...
const scenarios = caseFiles.map((file) => {
  const source = fs.readFileSync(path.join(casesDirectory, file), "utf8");
  ...
  return vm.runInNewContext(`(${objectSource})`, Object.create(null), {
    filename: file,
  });
});
...
console.log(`Exported ${scenarios.length} detective cases to ${outputFile}`);
